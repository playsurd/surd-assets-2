if (typeof locI18next === 'undefined') {
    const locI18next = { 
        init: function() { console.warn("locI18next was stubbed to prevent crash."); },
        // Add other missing functions as needed if they cause errors later
        // e.g., load: function() {}, 
    };
}
...
function setSystemName(name) {
    ...
    if (playerData) {
        ...
        if (!globalPlayerData[playerData.uuid]) {
            globalPlayerData[playerData.uuid] = {};
        }
...
function onUpdateSystemGraphic(name) {
  if (gameUiThemes.indexOf(name.replace(/'|\s$/g, '')) > -1) {
    ...
    // checks if locI18next and its init function exist before running
    if (typeof locI18next === 'object' && typeof locI18next.init === 'function') {
      locI18next.init(i18next, { ...locI18nextOptions, document: uiThemeModalContent })('.uiThemeItem.auto label');
    } else {
      console.warn("Skipped locI18next init in onUpdateSystemGraphic to prevent fatal crash.");
    }
