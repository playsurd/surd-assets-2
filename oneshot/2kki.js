...
function get2kkiLocationHtml(location, showDepth) {
  ...
  const urlTitleJP = location.urlTitleJP || (location.titleJP && location.titleJP.indexOf('ï¼š') > -1 ? location.titleJP.slice(0, location.titleJP.indexOf('ï¼š')) : location.titleJP);
    ...
    for (let ct of connTypes) {
      if (location.connType & ct) {
        ...
        if (connTypeParams && ct === ConnType.EFFECT) {
          ...
          // connTypeParams.paramsJP = connTypeParams.paramsJP.split(',').map(e => effectsJP[e]).join('ã€ã‹ã€Œ');
        }
