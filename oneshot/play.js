...
function resetCanvas() {
    console.log("resetCanvas() called. (Stub)");
    // This is often a function exposed by the engine, but we stub it to prevent crashes
    return true; 
}
function markMapUpdateInChat(mapId) {
}
function onLoad2kkiMap(mapId) {
}
function FS_createPreloadedFile() {
}
function preloadMapFiles(mapId) { }
function postRun() { }
function onRuntimeInitialized() { }
function playSound() { }
function allocate_audio_buffer() { return 0; }
function ___async_exit() {
    // Return value is an Asyncify internal state ID, typically 0 on success.
    return 0;
}
function ___async_enter() {
    // Return value is an Asyncify internal state ID, typically 0 on success.
    return 0; 
}
function locI18next(key) {
    // Returning the key itself is the safest fallback.
    return key; 
}
